...
(function ($) {
	...
	$(document).ready(function () {
		...
		if (
			$(".header-slider").length &&
			$(".header-slider").data("items") == "1"
		) {
			$(window).resize(function () {
		...
		$(".owl-carousel").each(function () {
			...
			if (nav == true) {
				left_arrow =
					left_arrow != "" ? '<i class="' + left_arrow + '"></i>' : "";
				right_arrow =
					right_arrow != "" ? '<i class="' + right_arrow + '"></i>' : "";
			}
		...
		$(".animated-owl-carousel").each(function () {
			...
			c_carousel.on("change.owl.carousel", function (event) {
				var animationDelay = $(event.target)
					.find(".stark-animated")
		...
		$('.navbar-nav a[href^="#"]').on("click", function (e) {
			...
			if (nav_height == $("nav.navbar").height()) {
				pos =
					parseInt($target.offset().top, 10) - $("nav.navbar").height() + 10;
			} else {
		...
		$(".panel-color-code").click(function () {
			...
			if ($(this).data("file")) {
				$("head").append(
					'<link rel="stylesheet" id="custom-color-panel" href="colors/' +
						file +
						'" type="text/css" />'
				);
			}
...
$(window).load(function () {
	...
	if ($(window).width() < 768) grid_col = 1;

	var net_width = Math.floor(
		(grid_width - gutter_size * (grid_col - 1)) / grid_col
	);
	$(".element-item").css({
		width: net_width + "px",
...
function initStarkContact() {
	...
	if ($("#starkGoogleMap").attr("data-map-style")) {
		...
		if (map_mode == "Aubergine") map_style_mode = map_style_obj.Aubergine;
		else if (map_mode == "Silver") map_style_mode = map_style_obj.Silver;
		else if (map_mode == "Retro") map_style_mode = map_style_obj.Retro;
		else if (map_mode == "Dark") map_style_mode = map_style_obj.Dark;
		else if (map_mode == "Aubergine") map_style_mode = map_style_obj.Night;
	}

	var LatLng = { lat: 51.508742, lng: -0.12085 };
	...
	var map = new google.maps.Map(
		document.getElementById("starkGoogleMap"),
		mapProp
	);
